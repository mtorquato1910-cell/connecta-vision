...
export const Route = createFileRoute("/produtos/categoria/$slug")({
  loader: ({ params }) => {
    ...
    return { cat, produtos: produtosPorCategoria(params.slug) };
  },
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.cat.nome} — Catálogo Conecta` },
          { name: "description", content: `${loaderData.cat.qtd} equipamentos veterinários da linha ${loaderData.cat.nome}.` },
        ]
      : [],
  }),
  notFoundComponent: () => (
...
function CategoriaPage() {
  const { cat, produtos } = Route.useLoaderData();
  return (
    <SiteShell>
