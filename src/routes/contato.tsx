  ...
  head: () => ({
    meta: [
      { title: "Contato | Conecta Equipamentos Veterinários, Distribuidor Shinova" },
      {
        name: "description",
        content:
          "Fale com a equipe técnica e comercial da Conecta por WhatsApp, telefone ou e-mail. Atendemos clínicas e hospitais em todo o Brasil, com resposta em até 4 horas úteis.",
      },
      { property: "og:title", content: "Fale com a Conecta" },
      {
        property: "og:description",
        content:
          "Atendimento técnico e comercial em todo o Brasil, com resposta em até 4 horas úteis.",
      },
    ],
  }),
...
function ContatoPage() {
  ...
  const { ordem } = usePageSecoes("contato");
  ...
  const blocos: Record<string, ReactNode> = {
    header: (
      <section className="container-edge pt-16 md:pt-24 pb-12">
        <Reveal>
          <span className="eyebrow">{texto("contato.eyebrow", "Fale com a gente")}</span>
          <h1 className="mt-5 font-serif text-5xl md:text-7xl text-ink leading-[1.02] max-w-4xl">
            {texto("contato.titulo", "Fale com gente que entende de equipamento veterinário.")}
          </h1>
        ...
        <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Info
            ...
            t="Telefone"
            v={telefone}
            href={`tel:+${c.telefone_principal_raw || SITE.phoneRaw}`}
            cta="Ligar"
          />
          <Info
            icon={Mail}
...
function Info({
  ...
  const body = (
    <div className="bg-paper border border-line rounded-3xl p-6 h-full transition hover:border-conecta-blue/30 hover:-translate-y-1">
      ...
      <div className="mt-5 font-mono text-[11px] tracking-[0.18em] uppercase text-ink-soft">
        {t}
      </div>
