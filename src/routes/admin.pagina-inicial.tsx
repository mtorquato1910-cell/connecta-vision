...
import { ArrowDown, ArrowUp, Eye, EyeOff, Home, RotateCcw, Save, Star } from "lucide-react";
...
const PAGINAS: Array<"home" | PaginaComSecoes> = ["home", "sobre", "solucoes", "contato"];
...
function AdminPaginaInicialPage() {
  const [paginaSel, setPaginaSel] = useState<"home" | PaginaComSecoes>("home");
  ...
  const persistSecoes = (secoes: SecaoConfig[]) => saveMut.mutate({ ...config, secoes });
  ...
  const destaquesDirty = JSON.stringify(destaques) !== persistedDestaqueKey;
  ...
  return (
    <div>
      ...
      {/* Seletor de página: home + demais páginas com seções configuráveis */}
      <div className="px-4 sm:px-6 md:px-10 pt-5">
        <nav className="bg-paper border border-line rounded-full p-1 inline-flex gap-1 max-w-full overflow-x-auto">
          {PAGINAS.map((p) => (
            <button
              key={p}
              onClick={() => setPaginaSel(p)}
            ...
            >
              {p === "home" ? "Home" : PAGE_LABEL[p]}
            </button>
          ))}
        </nav>
      </div>

      {paginaSel !== "home" && <PageSecoesEditor pagina={paginaSel} />}
        ...
        <div className="px-4 sm:px-6 md:px-10 py-5 sm:py-6 md:py-8 max-w-5xl space-y-8">
          ...
          <section>
            ...
            <p className="text-sm text-ink-soft mb-4">
              Use as setas para reordenar e o botão de olho para ativar/desativar cada bloco.
            </p>
            ...
            <div className="flex items-start justify-between flex-wrap gap-2 mb-3">
              <div>
                ...
                <p className="text-sm text-ink-soft mt-1">
                  {destaques.length}{" "}
                  {destaques.length === 1 ? "produto selecionado" : "produtos selecionados"}.
...
// ─── Editor de seções de páginas (Sobre / Soluções / Contato) ────────────────
function PageSecoesEditor({ pagina }: { pagina: PaginaComSecoes }) {
  const qc = useQueryClient();
  const chave = PAGE_CONFIG_KEY[pagina];

  const { data: secoes = resolvePageSecoes(pagina, null) } = useQuery({
    queryKey: ["admin-page-secoes", pagina],
    queryFn: async () => {
      const rows = (await getConteudoPublic()) as Array<{ chave: string; valor: unknown }>;
      const row = rows.find((r) => r.chave === chave);
      return resolvePageSecoes(pagina, (row?.valor as PageSecoesConfig) ?? null);
    },
    initialData: resolvePageSecoes(pagina, null),
  });

  const saveMut = useMutation({
    mutationFn: (next: PageSecao[]) => upsertConteudo({ data: { chave, valor: { secoes: next } } }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-page-secoes", pagina] });
      qc.invalidateQueries({ queryKey: ["page-secoes", pagina] });
    },
    onError: (e: any) => toast.error(e?.message ?? "Erro ao salvar."),
  });

  const ordered = secoes.slice().sort((a, b) => a.ordem - b.ordem);
  const persist = (list: PageSecao[]) =>
    saveMut.mutate(list.map((s, i) => ({ ...s, ordem: i + 1 })));
  ...
  const toggle = (id: string) =>
    persist(ordered.map((s) => (s.id === id ? { ...s, ativa: !s.ativa } : s)));

  return (
    <div className="px-4 sm:px-6 md:px-10 py-5 sm:py-6 md:py-8 max-w-5xl">
      <p className="text-sm text-ink-soft mb-4">
        Seções da página <strong>{PAGE_LABEL[pagina]}</strong>. Use as setas para reordenar e o olho
        para mostrar/ocultar cada bloco no site.
      </p>
      <div className="space-y-2">
        {ordered.map((s, i) => (
          <div
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-serif font-normal text-base sm:text-lg text-ink">{s.label}</h3>
              <p className="text-xs text-ink-soft line-clamp-1 mt-0.5">{s.descricao}</p>
            </div>
            <button
              onClick={() => toggle(s.id)}
              aria-label={s.ativa ? "Ocultar seção" : "Mostrar seção"}
              className={`h-9 w-9 rounded-md flex items-center justify-center transition-colors ${
                s.ativa
                  ? "text-emerald-700 bg-emerald-50 hover:bg-emerald-100"
                  : "text-slate-500 bg-slate-100 hover:bg-slate-200"
